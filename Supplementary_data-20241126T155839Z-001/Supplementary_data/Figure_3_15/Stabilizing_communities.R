...
Exps_st_c <- Exps_st_c_30_42  <- read_excel("Stabilizing_communities.xlsx")
View(Exps_st_c)

Exps_st_c_v1 <- Exps_st_c %>%
  pivot_longer(names_to = "Condition", values_to = "OD_578", cols = -Time_h)
# Exps_st_c_v1
View(Exps_st_c_v1)


Exps_st_c_v2 <- separate(Exps_st_c_v1, Condition, 
                    c("Exp_day",
                      "Community",
                      "Transfer_medium",
                      "Zn_condition", 
...
Exps_st_c_v2$OD_578 <- as.numeric (Exps_st_c_v2$OD_578) #Because OD_578 values are character


Exps_st_c_v2$Community <- factor(Exps_st_c_v2$Community, levels = c("control",
                                                          "C1",
                                                          "C2",
                                                          "C3",
                                                          "C4",
...
Exps_st_c_v2$drug_conc <- factor(Exps_st_c_v2$drug_conc, levels = c("nodrug",
                                                          "1.74",
                                                          "10",
                                                          "0",
                                                          "1.8",
...
Exps_st_c_v2 <- Exps_st_c_v2 %>% mutate(drug_conc =  recode (drug_conc,
                                                   "nodrug" = "blank",
                                                   "1.74" = "17.4 uM Zn",
                                                   "10" = "50 uM Zn",
                                                   "0" = "0 uM TPEN",
...
Exps_st_c_v2 <- Exps_st_c_v2 %>% mutate(Community =  recode (Community,
                                                   "control" = "blank"))

Exps_st_c_v2 <- Exps_st_c_v2 %>% mutate(Tech_Repl =  recode (Tech_Repl,
                                                   "TechRep1" = "TR1",
                                                   "TechRep2" = "TR2"))



# filtered_data <- Exps_st_c_v2 %>%
# filter(!(Exp_day == "Exp45"))
...
Exps_st_c_v3 <- Exps_st_c_v2 %>% group_by(Community, Transfer_medium, Zn_condition, Tech_Repl, Biol_Repl, Well_number) %>%
  mutate(blank = Time_h ==0)

#Exps_st_c_v3 <- filtered_data %>% group_by(Community, Transfer_medium, Zn_condition, Tech_Repl, Biol_Repl, Well_number) %>%
#  mutate(blank = Time_h ==0)

# View(Exps_st_c_v3)

Exps_st_c_v4 <- Exps_st_c_v3 %>%
...
Exps_st_c_v5 <- Exps_st_c_v4 %>% group_by(Time_h, Community, Transfer_medium, drug_conc) %>%
  summarise(ODmean = mean(OD), sdOD = sd(OD), n())

Exps_st_c_v5 <- Exps_st_c_v5 %>% mutate(drug_conc =  recode (drug_conc,
...
Exps_st_c_v5  %>% ggplot(aes(x = Time_h,
                        y = ODmean,
                        color = drug_conc,
                        fill = drug_conc
)) +
...
# Assuming Exps_st_c_v4 or Exps_st_c_v5 contains your cleaned commmunity's data
# Filter the data to exclude blanks and unnecessary data points

# Ensure OD values are numeric and calculate AUC
Exps_st_c_v5$ODmean <- as.numeric(Exps_st_c_v5$ODmean)

#To normalize according to 17.4 uM Zn condition
Exps_st_c_v4 <- Exps_st_c_v4 %>% filter (Community != "blank", drug_conc != "blank") %>% mutate(control = drug_conc == "0 uM TPEN", ODc0 = OD)

# table(Exps_st_c_v4$Biol_Repl)
...
Exps_st_c_v4 <- Exps_st_c_v4 %>% mutate(f_experiment_number = as.factor(Exp_day))

time_cutoffs <- determineTimeCutoffs(Exps_st_c_v4) %>%
  mutate(f_experiment_number = as.factor(Exp_day))
print(time_cutoffs)

Exps_st_c_v4_control <- Exps_st_c_v4 %>% 
  filter(control) %>% 
  group_by(f_experiment_number, Exp_day, Community, Time_h) %>%
  summarise(OD = median(OD))

Exps_st_c_v4 <- Exps_st_c_v4 %>% left_join(time_cutoffs) %>% filter(Time_h <= cutoff_time) %>%
  select(-cutoff_time)
...
AUCs <- Exps_st_c_v4 %>% group_by(Community,Exp_day,Biol_Repl, Tech_Repl,
...
AUCs %>%
  ggplot(aes(x = as.factor(drug_conc), y = AUC, color = Community)) +  # Convert drug_conc to factor for discrete x-axis
  geom_boxplot() +
  labs(title = "Boxplots of AUC by Drug Concentration and Community",
       x = "Drug Concentration (µM)",
       y = "AUC") +
  theme_minimal() +
