...
source("~/ITQB/Omics/bin/Helper_functions.R")
```
...
out_dir = "~/ITQB/Omics/dieciseis_out"
```
...
# Referen
ref_dir = "~/ITQB/Omics/reference_files"
...
filtered_trimmed = filterAndTrim(fwd = forward_reads, 
              ...
              trimLeft = c(50,50), # I changed 23, 24 to 50, 50. Trim nucleotides corresponding to primer sequences. To check align primers to raw seqs
              truncLen=c(240, 240), # Truncation position of F and R
              ...
              compress=TRUE, 
              verbose=TRUE, 
              multithread=FALSE) #Changed multithread to TRUE to fasten the process
```
...
forward_errors = learnErrors(forward_filt_reads, 
                             nbases = 1e8, 
                             randomize=TRUE,
                             multithread=FALSE, #Changed multithread to TRUE to help speed up the process
                             verbose = TRUE)
...
reverse_errors = learnErrors(forward_filt_reads, 
                             nbases = 1e8, 
                             randomize=TRUE,
                             multithread=FALSE, #changed from false to true to help speed up the process
                             verbose = TRUE)
```
...
forward_dada = map(forward_derep, 
                   function(derep_obj){dada(derep_obj, err=forward_errors, multithread=FALSE, verbose = TRUE)}) #Changed multithread to TRUE to sped up calculatiton time
```
...
reverse_dada = map(reverse_derep, 
                   function(derep_obj){dada(derep_obj, err=reverse_errors, multithread=FALSE, verbose = TRUE)}) #Changed multithread to TRUE to sped up calculatiton time
...
Min_len = 246 #I changed 250 with 286
Max_len = 248 #I changed 254 with 288
...
seq_table_dechimered <- removeBimeraDenovo(seq_table,
                                    method="consensus", 
                                    multithread=FALSE, #changed multithread to TRUE to help speeding up the process
                                    verbose=TRUE)
...
merged_taxonomy = assignTaxonomy(seq_table_dechimered, 
                       ...
                       minBoot=70,
                       tryRC = TRUE,
                       multithread=FALSE) #Chaanged multithread to TRUE to help speeding up the process
...
# Construct phylogeny (not required for us)
...
out_asv_file = paste(file_prefix, "ASV_table_240_240.tsv", sep = "_") %>% 
  file.path(out_dir, .)

out_tax_file = paste(file_prefix, "ASV_taxonomy_240_240.tsv", sep = "_") %>% 
  file.path(out_dir, .)

out_tree_file = paste(file_prefix, "ASV_tree_240_240.tre", sep = "_") %>% 
  file.path(out_dir, .)
