...
All_R_mGAM_clean <- Exp3to25_v4_mGAM  %>%  filter (
    ...
    !(Strain == "12016" &  Exp_number == "Exp14"), #Apparently I already deleted all conc curves except blank--> delete Exp14 (completely)
...
AUCs_M8_mean$short <- factor(AUCs_M8_mean$short, levels = c("T. ramosa",
                                                            "S. salivarius",
                                                            "S. copri",
                                                            "R. intestinalis",
                                                            "P. vulgatus",
...
AUCs_mGAM_mean$short <- factor(AUCs_mGAM_mean$short, levels = c("T. ramosa",
                                                            "S. salivarius",
                                                            "S. copri",
                                                            "R. intestinalis",
                                                            "P. vulgatus",
...
AUCs_mGAM_mean %>% 
  filter (conc != "blank") %>% filter (short != "L. murinus") %>%
  ggplot(aes(conc, short, fill = AUCsmean)) +
  geom_raster() +
  #geom_text(data = pvs %>% filter(qv_rescue < 0.01), label = "*") +
  scale_fill_gradient2(low = "black", high = "blueviolet") +
...
####_____IC25_plot_mGAM______________
# Filter out blank concentrations
AUCs_mGAM_mean_1 <- AUCs_mGAM_mean %>% filter(conc != "blank")

# Convert `conc` to character first if it's still a factor
AUCs_mGAM_mean_1 <- AUCs_mGAM_mean_1 %>%
  mutate(conc = as.character(conc))

# Define the concentrations you want to keep
desired_conc_mGAM <- c("0 uM", "54 uM", "60 uM", "70 uM", "80 uM", 
                  "90 uM", "100 uM", "110 uM", "120 uM", "130 uM", "140 uM")

# Filter for only the desired concentrations
AUCs_mGAM_mean_1 <- AUCs_mGAM_mean %>%
  filter(conc %in% desired_conc_mGAM)

# Remove " uM" suffix and convert `conc` to numeric
AUCs_mGAM_mean_1 <- AUCs_mGAM_mean_1 %>%
...
# Create the plot
AUCs_mGAM_mean_1 %>%
  ggplot(aes(x = conc, y = AUCsmean, color = AUCsmean <= 0.75)) +
  geom_point() +
  scale_color_manual(values = c("TRUE" = "red", "FALSE" = "blue")) +
  # geom_text(data = subset(AUCs_mGAM_mean_1, conc == first_red_point),
...
# Reorder 'short' according to the custom strain order
IC25max_2_mGAM <- IC25_2_mGAM %>%
  mutate(short = factor(short, levels = c("E. coli O127:H6",
                                          "E. coli UTI89",
                                          "E. coli Nissle 1917",
                                          "E. coli O83:H1",
                                          "L. saccharolytica",
                                          ...
                                          "C. comes",
                                          "L. fermentum",
                                          "C. difficile")))  # Set the custom order



IC25max_2_mGAM <- IC25max_2_mGAM %>%
  group_by (short) %>%
  slice(which.max(AUCsmean_clean))

IC25max_2_mGAM %>%
  ggplot(aes(y = short, x = conc, color = Family)) +
  geom_point( size = 3) +
  #facet_grid( .~ metal) +
  labs(x = "TPEN_conc, µM (IC25)", y = "Strains") +
  theme_bw()

#_____IC25_plot_M8______________
# Filter out the positive control - 17.4 uM Zn
AUCs_M8_mean_1 <- AUCs_M8_mean %>% filter(conc != "17.4 uM Zn")

# Convert `conc` to character first if it's still a factor
AUCs_M8_mean_1 <- AUCs_M8_mean_1 %>%
  mutate(conc = as.character(conc))

# Define the concentrations you want to keep
desired_conc_M8 <- c("0 uM", "18 uM", "24 uM", "30 uM", "36 uM", 
                       "42 uM", "48 uM", "54 uM", "60 uM", "70 uM", "80 uM")

# Filter for only the desired concentrations
AUCs_M8_mean_1 <- AUCs_M8_mean %>%
  filter(conc %in% desired_conc_M8)
...
# Filter the dataframe to get the TPEN_con value for the first red point
first_red_point <- AUCs_M8_mean_1 %>% group_by(short) %>%
  filter(AUCsmean <= 0.75) %>%
  mutate(IC25 = AUCsmean <= 0.75) %>%
  filter (IC25 == TRUE)
...
IC25max_2_M8 <- IC25_2_M8 %>%
  mutate(short = factor(short, levels = c("E. faecalis",
                                          "E. coli O127:H6",
                                          "E. coli UTI89",
                                          "E. coli O83:H1",
                                          "S. enterica",
